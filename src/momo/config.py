...
def _load_system_prompt_config(user_name: str) -> SystemPromptConfig:
    system_prompt_config = SystemPromptConfig(user_name=user_name)
...
def load_momo_config() -> MomoConfig:
    momo_config = MomoConfig(
        ...
        prompt=prompt.construct_system_prompt_from_config(
            _load_system_prompt_config(user_name="Niklas")
        ),
    )
